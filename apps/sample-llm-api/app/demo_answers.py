...
def build_demo_context(request: ChatRequest) -> list[str]:
    base_context = [
        ...
        "OLLY는 요청마다 요청 ID와 트레이스 ID를 만들고, 운영자 대시보드의 최근 요청에서 추적합니다.",
        ...
        "토큰 사용량이 많으면 총 토큰과 총 비용이 증가합니다.",
        "실패 요청은 status=error로 기록되고 에러율 지표에 반영됩니다.",
    ]
    scenario_context = {
        "normal": [
            "일반 요청 시나리오는 정상 요청입니다. 대시보드에는 낮은 응답 시간과 정상 상태로 표시됩니다.",
        ],
        "slow_retrieve": [
...
def build_demo_answer(request: ChatRequest) -> str:
    ...
    if request.scenario == "slow_retrieve":
        return (
            ...
            "대시보드에서는 이 요청의 트레이스 상세에서 RAG 검색 구간이 가장 길게 보일 것입니다."
        )
    ...
    if request.scenario == "high_token" or "토큰" in question or "비용" in question:
        return (
            ...
            "그래서 대시보드에서는 총 토큰과 총 비용이 함께 올라간 것으로 보입니다."
        )
    ...
    if request.scenario == "error" or "실패" in question or "에러" in question:
        return (
            ...
            "OLLY는 이런 실패도 그냥 버리지 않고 요청 ID, 트레이스 ID, 오류 지표로 기록합니다.\n\n"
            "그래서 운영자는 실패한 요청을 최근 요청에서 실패 상태로 보고, 같은 트레이스 ID로 어느 단계에서 실패했는지 추적할 수 있습니다."
        )
    ...
    if "rag" in question.lower() or "openai" in question.lower() or "느린" in question:
        ...
        "즉, 단순히 'LLM이 느리다'고 보는 것이 아니라 어느 단계가 전체 응답 시간을 만든 것인지 구분할 수 있습니다."
        )
    ...
    return (
        ...
        "그래서 운영자는 감으로 추측하지 않고 요청 ID와 트레이스 ID를 기준으로 원인을 확인할 수 있습니다."
    )
